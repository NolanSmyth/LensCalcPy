...
def rho_thick_mw(
    ...
    if r > rdBreak:
        result = (
            rho_thick_Sol
            * np.exp(-((r - rsol) / rthick))
            * np.exp(-(np.abs(z) / zthickSol))
        )
    else:
        result = (
            rho_thick_Sol
            * np.exp(-((rdBreak - rsol) / rthick))
            * np.exp(-(np.abs(z) / zthickSol))
        )
...
def rsf(xp, yp, zp):
    rs = (
        ((xp / x0) ** cperp + (yp / y0) ** cperp) ** (cpar / cperp) + (zp / z0) ** cpar
    ) ** (1 / cpar)
    return rs
...
class Ffp(Lens):
    ...
    def pl_norm(self, p):
        ...
        return N_ffp / abs(
            nquad(
                ...
                opts={"points": [self.m_min, self.m_min * 1e3, self.m_min * 1e5]},
            )[0]
        )

    def differential_rate_integrand(
        self,
        umin,
        d,
        mf,
    ...
    def differential_rate(self, t, integrand_func, finite=False):
        ...
        for i in range(num):
            ...
            if i == 0:  # for the first point
                ...
                dm = (
                    (mf_values[i + 1] - mf_values[i])
                    + (mf_values[i] - mf_values[i - 1])
                ) / 2
            else:  # for the last point
                dm = mf_values[i] - mf_values[i - 1]
            if finite:
                single_result, error = dblquad(
                    integrand_func,
                    0,
                    ds,
                    lambda d: 0,
            ...
            result += (
                single_result * (mf**-self.p) * dm
            )  # multiply by mass function and by dm
        result *= self.Z  # normalization
        return result

    def differential_rate_integrand_mw(self, umin, d, mf, t, finite=False):
        return self.differential_rate_integrand(
            umin,
            d,
            mf,
            t,
            ...
            finite=finite,
            density_func_uses_d=True,
        )

    def differential_rate_mw(self, t, finite=False):
        return self.differential_rate(
            t, self.differential_rate_integrand_mw, finite=finite
        )
    ...
    def differential_rate_m31(self, t, finite=False):
        return self.differential_rate(
            t, self.differential_rate_integrand_m31, finite=finite
        )
    ...
    def differential_rate_total(self, t, finite=False):
        return self.differential_rate_mw(t, finite=finite) + self.differential_rate_m31(
            t, finite=finite
        )
